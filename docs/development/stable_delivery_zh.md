...
| C 长期容量 | 已审核的 `runtime_capacity.py`，1/4/8 会话 4096 根正在运行 | 进行中；需核对 RSS、长期延迟、缓存淘汰与恢复成本的完整结果 |
| D 迁移体验 | 真实 ADX/DI 初译失败、迁移提示、修正执行及 80 根外部对照 | 已完成本代表性案例；不声称所有语料可迁移 |
| E 发行包 | 同一 wheel 九平台配置；Windows 3.12/3.13 已安装 wheel 的五类实际流程通过 | 部分完成；选定提交的远端运行与完整门禁未验证 |
